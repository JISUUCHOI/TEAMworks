					...
					<c:choose>
						...
						<c:when test="${ !empty asc }">
							<c:url value="searchCallback.rap" var='searchUrl'>
								...
								<c:param name="storage" value="결재회수함" />
							</c:url>
							<button class="page able" onclick="location.href='${ searchUrl }'">&lt;</button>
						</c:when>
						<c:otherwise>
							<button class="page able" onclick="location.href='callbackList.rap?currentPage=${ pi.currentPage - 1 }&storage=결재회수함'">&lt;</button>
						</c:otherwise>
					</c:choose>
					...
					<c:forEach var="p" begin="${ pi.startPage }" end="${ pi.endPage }">
						<c:choose>
							...
							<c:otherwise>
								<c:choose>
									<c:when test="${ empty asc }">
										<button class="page able" onclick="location.href='callbackList.rap?currentPage=${ p }&storage=결재회수함'">${ p }</button>
									</c:when>
									<c:otherwise>
										<c:url value="searchCallback.rap" var='searchUrl'>
